...
uniqueness = "DRM-Widevine-" + str(random.randint(0,9999))
content_key_policy_name = 'CommonEncryptionCencDrmContentKeyPolicy_2022_05_25_Widevine'
...
# Add the license type configurations for Widevine to the policy
options = [
  ContentKeyPolicyOption(
    ...
    if paths.streaming_paths:
      ...
      for path in paths.streaming_paths:
        for format_path in path.paths:
          ...
          print(f"Click to playback in AMP player: https://ampdemo.azureedge.net/?url={manifest_path}&widevine=true&token=Bearer%20{jwt_token}")
