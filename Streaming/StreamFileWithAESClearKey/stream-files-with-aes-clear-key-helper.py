...
content_key_policy_name = 'Predefined_ClearKey'
...
async def create_or_update_content_key_policy(policy_name,token_signing_key):
    ...
    clearkey_config = ContentKeyPolicyClearKeyConfiguration()

    # Add the license type configurations to the policy
    options = [
        ContentKeyPolicyOption(
            configuration=clearkey_config,
    ...
    create_content_policy = await client.content_key_policies.create_or_update(
        ...
        parameters=ContentKeyPolicy(
            description="AESClearKey Content Key Policy",
            options=options
        )
    )
...
async def create_streaming_locator(output_asset_name,locator_name,content_key_policy_name):
    locator_name = f"locator-{uniqueness}"
    streaming_locator = StreamingLocator(asset_name=output_asset_name, streaming_policy_name="Predefined_ClearKey", default_content_key_policy_name=content_key_policy_name)
...
async def get_streaming_urls(locator_name, token):
    ...
    for path in paths.streaming_paths:
        for format_path in path.paths:
            ...
            print("For AES ClearKey testing, please open the link in the Microsoft Edge Browser.")
            print(f"Click to playback in AMP player: https://ampdemo.azureedge.net/?url={manifest_path}&aes=true&aestoken={token}")
