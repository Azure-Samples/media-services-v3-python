...
uniqueness = "streamDRM-" + str(random.randint(0,9999))
input_extension = ''
blob_name = ''
content_key_policy_name = 'CommonEncryptionCencDrmContentKeyPolicy_2022_05_25'
...
transform_name = 'ContentAwareEncodingStreamFilesDRM'
...
myTransform.description="Transform with Stream Files"
...
wide_vine_config = ContentKeyPolicyWidevineConfiguration(
...
options = [
  ...
  ContentKeyPolicyOption(
    configuration = wide_vine_config,
    restriction = restriction
  )
]
...
if output_asset is not None:
  streamingLocator = StreamingLocator(asset_name=out_asset_name,streaming_policy_name="Predefined_MultiDrmCencStreaming", default_content_key_policy_name=content_key_policy_name)
    ...
    if paths.streaming_paths:
      ...
      for path in paths.streaming_paths:
        for formatPath in path.paths:
          ...
          print("For Widevine testing, please open the link in the Chrome Browser.")
          print(f"Click to playback in AMP player: https://ampdemo.azureedge.net/?url={manifest_path}&playready=true&widevine=true&token=Bearer%20{jwt_token}")
